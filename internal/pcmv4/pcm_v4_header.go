...
// ProtocolVersion is the audio protocol this addon speaks, and the only one
// it reads.
//
...
//
// That matters here: each recorder channel holds a session open for the life of
// the process, so the saving is on every byte of a stream that never stops,
