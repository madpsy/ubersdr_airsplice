...
// DecodePacketLE is DecodePacket in the shape this addon works in:
// little-endian int16 bytes plus the stream parameters.
//
// Little-endian is what the WAV recorder, the audio preview and the FFT already
// want, and it is what the codec produces, so unlike the versions 1-3 path
// there is no byte swap. Those carried radiod's big-endian samples and
// reversed them per packet.
//
